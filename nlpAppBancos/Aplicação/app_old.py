...
if st.button('Calcular probabilidades', 'button'):
    ...
    if tamanho_resenha < 2:
        ...
        if tamanho_resenha >= 20:
            st.subheader('Análise sintática e morfológica da resenha')
            st.write('Atenção: o uso do painel a esquerda recarrega a aplicação')
            st.write('Apesar de análise estar em inglês, esta foi realizada no idioma português')
            doc = pln(resenha)
            visualize_parser(doc)
        else:
            st.write('Resenhas com pelo menos 20 caracteres apresentam análise sintática e morfológica')